		...
		for (int contMetodoAG = 0; contMetodoAG < 4; contMetodoAG++) {
			...
			for (int i = 0; i < n; i++) {
				...
				for (int j = 0; j < fitnessMedio.length; j++) {
					...
					if(auxValorConvergencia == melhoresFitness[j]){
						cont++;
					}else{
					...
					if(cont==5){
						geracaoDaConvergenciaAuxliar[i] = j-cont; 
					}
				}
			...
			System.out.println("fit medio da população --> "+fitnessMedioMedioDaPopulacao[contMetodoAG]);
			System.out.println("media fit solução --> "+mediaDoMelhorIndividuoSolucao[contMetodoAG]);
			System.out.println("media geracao de congergencia --> "+mediaGeracaoDaConvergencia[contMetodoAG]);
			System.out.println("____________________________________");
		}
		
		GraficoDeBarras g = new GraficoDeBarras();
